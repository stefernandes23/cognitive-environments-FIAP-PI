...
def configure_aws():
    return boto3.client(
        'rekognition',
        aws_access_key_id=st.secrets["aws"]["AWS_ACCESS_KEY_ID"],  # Modificado aqui
        aws_secret_access_key=st.secrets["aws"]["AWS_SECRET_ACCESS_KEY"],  # E aqui
...
def analyze_face(image_bytes):
    try:
        ...
        st.error("Erro na conexão com AWS. Verifique as credenciais.")
